...
function customJSONError($errno, $errstr) {
  echo json_encode(array('status' => "err", 'text' => "\nError: [$errno] {$errstr}. Died."));
  die();
}

set_error_handler("customJSONError");
...
class edb{
	...
	public function __construct($host, $user=0, $pass=0, $db=0){
		...
		if(is_array($data)){
			...
			$this->connection = mysql_connect($host, $user, $pass) or customJSONError(23, mysql_error());
			mysql_select_db($db) or customJSONError(23, mysql_error());
	...
	public function q($a,$c=0,$t=30){
		...
		if($c && is_file($cacheFile) && (time()-filemtime($cacheFile))<$t){
			...
			$start	=	microtime(1);
			$this->res = array();
			$q = mysql_query("$a", $this->connection) or customJSONError(23, mysql_error());
	...
	private function setCache($file,$result,$q,$o=true){
		$fh = fopen($file, 'w') or customJSONError(23, "can't open file");
  ...
  //update row or rows, $db->checkTableExists($tableName);
  public function checkTableExists($a,$b, $c=0,$t=30) {
    $q = "SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = '{$a}' AND table_name = '{$b}'";
